...
def compute_regex(exp_t):
    ...
    elif exp_t.charType == charType.ALT:
        return do_alt(exp_t)
    else:
...
def do_alt(exp_t):
    ...
    #print(left_nfa[0].next_state, right_nfa[0].next_state)
    print(exp_t.left.value, exp_t.right.value)
    left_nfa[0].next_state['$'] = [left_nfa[0]]
    #left_nfa[1] = left_nfa[0]
    right_nfa[0].next_state['$'] = [right_nfa[0]]
    #print(left_nfa[0].next_state, right_nfa[0].next_state)
    return left_nfa[0], right_nfa[0]
...
def load_regex():
    with open('laba3/begin_regex.json', 'r') as inpjson:
        regex = json.loads(inpjson.read())
    return regex

def output_nfa():
    global nfa
    with open('laba3/nfa.json', 'w') as outjson:
